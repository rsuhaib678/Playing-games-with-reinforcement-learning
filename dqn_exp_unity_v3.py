...
def main(test=False, chkpt=None):
    ...
    engine_config_channel.set_configuration_parameters(
        width=1800, height=900, time_scale=40.0
    )
    ...
    try: 
        while True:
            ...
            if random() < eps:
                ...
                action_tensor = m(torch.Tensor(last_observation))
            ...
            actiontuple.add_discrete(action)
            # Take action
...
if __name__ == '__main__' :
    # main(True, "models/81104.pth")
    wandb.init(project="dqn-autopilot-check", name="dqn-autopilot-check")
    main()
